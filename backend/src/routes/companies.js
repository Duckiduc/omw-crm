...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [companyResult, contactsResult, dealsResult] = await Promise.all([
      db.query('SELECT * FROM companies WHERE id = $1 AND user_id = $2', [id, req.user.id]),
      db.query('SELECT * FROM contacts WHERE company_id = $1 AND user_id = $2 ORDER BY created_at DESC', [id, req.user.id]),
      db.query(`
...
router.post('/', [
  ...
  try {
    ...
    const result = await db.query(`
      ...
      RETURNING *
    `, [name, industry || null, website || null, phone || null, email || null, address || null, notes || null, req.user.id]);
